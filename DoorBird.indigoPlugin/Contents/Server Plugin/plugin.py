...
class Plugin(indigo.PluginBase):
    ...
    def startup(self):
        ...
        if 'pysodium' not in sys.modules:
            missing1 = True
        if 'pygtail' not in sys.modules:
            missing2 = True

        if missing1 or missing2:

            self.logger.error("")
            self.logger.error("########## Dorrbird Plugin Action Required ###########")
            self.logger.error("")
            self.logger.error("Please open Terminal on your Indigo Server Mac & type the following commands:")
            self.logger.error("")
            if missing1:
                self.logger.error("    pip3 install pysodium")
            if missing2:
                self.logger.error("    pip3 install pygtail")
            self.logger.error("")
            self.logger.error("######################################################")
            self.logger.error("")
            ...
            plugin = indigo.server.getPlugin("net.dowles.doorbird")
            if plugin.isEnabled():
                plugin.restart()
    ...
    def actionControlDevice(self, action, dev):
        ...
        if action.deviceAction in (indigo.kDeviceAction.Lock, indigo.kDeviceAction.TurnOff):
            ...
            if doorbirdID != "None":
                ...
                if isSensorControl == False:
                    lockTimer = threading.Timer(int(dev.pluginProps["autoOff"]), dev.updateStateOnServer,
                                                ['onOffState', True])
                    lockTimer.start()
            else:
        ...
        if typeId == "doorbird":
            ...
            if valuesDict["enableSSIRActivation"]:
                ...
                if valuesDict["ssCameraNum"].isdigit() == False:
                    errorDict[
                        "ssCameraNum"] = "Security Spy camera number must be a number. Look in Security Spy to find the camera number corresponding to this Doorbird."
        ...
        for dev in indigo.devices.iter("self"):
            if dev.deviceTypeId == "doorbird":
                if typeId == "motion":
                    ...
                    if Doorbird.instancesID[dev.id].motionDeviceID == None or Doorbird.instancesID[
                        dev.id].motionDeviceID == int(devId):
                        return_list.append((str(dev.id), dev.name))
                if typeId == "doorbell":
                    # Only present Doorbirds that have no motion device assigned
                    if Doorbird.instancesID[dev.id].doorbellDeviceID == None or Doorbird.instancesID[
                        dev.id].doorbellDeviceID == int(devId):
                        return_list.append((str(dev.id), dev.name))
...
class Doorbird(object):
    ...
    def __init__(self, indigoID=None, ip=None, user=None, password=None, motionDeviceID=None, doorbellDeviceID=None,
        ...
        self.keepAlive = int(
            (datetime.datetime.now() - datetime.timedelta(seconds=31)).strftime('%s'))  # now minus 31 seconds
        ...
        if "enableSSIRActivation" in indigo.devices[
            self.indigoID].pluginProps:  # required to prevent error when upgrading the plugin.
    ...
    def check_status(self):
        ...
        if response[0] == False and response[1] == 401:
            ...
            self.logger.error(
                indigo.devices[self.indigoID].name + ": Unable to connect to Doorbird. HTML status code: " + str(
                    response[1]))
            status = False
            ...
            try:
                ...
                if onOffState != alive:
                    ...
                    if alive == True:
                        ...
                        dev.updateStateOnServer('doorbirdLastUpdate',
                ...
                if os.path.isfile(indigo.devices[self.indigoID].pluginProps["ssWebLogPath"]):
                    if indigo.variables[isDaylightId].value == "false":
                        for line in Pygtail(indigo.devices[self.indigoID].pluginProps["ssWebLogPath"]):
                            if (("cameraNum=" + indigo.devices[self.indigoID].pluginProps["ssCameraNum"]) in line) and (
                                    indigo.devices[self.indigoID].states["doorbirdOnOffState"] == True):
                                if (lastTriggered == None) or (time.time() - lastTriggered > 30):
                                    self.logger.debug(
                                        indigo.devices[self.indigoID].name + ": Security Spy web log criteria detected")
                                    self.turn_light_on()
                                    lastTriggered = time.time()
                else:
                    if (lastError == None) or (
                            time.time() - lastError > 300):  # only send error messages every 5 minutes so as not to spam the log
                        lastError = time.time()
                        self.logger.error(indigo.devices[self.indigoID].name + ": Invalid Security Spy web log path: " +
                                          indigo.devices[self.indigoID].pluginProps["ssWebLogPath"])
            except:
                if (lastError == None) or (
                        time.time() - lastError > 300):  # only send error messages every 5 minutes so as not to spam the log
                    lastError = time.time()
    ...
    def energize_relay(self, relayID):
        ...
        if self.check_status() == True:
            ...
            try:
                ...
                self.logger.info(
                    indigo.devices[self.indigoID].name + ": Relay " + str(relayID) + " energize command sent")
            except:
                self.logger.error(indigo.devices[self.indigoID].name + ": Unable energize relay " + str(relayID))
        else:
            self.logger.error(
                indigo.devices[self.indigoID].name + ": Relay " + str(relayID) + " energize command not sent")
